...
interface IVolumeBot {
    function fire() external;
}

/// @title CabalEmissions V5
/// @notice V5 = V4 + VolumeBot hook. Every user-facing action (deposit / withdraw /
///         harvest / harvestAll / emergencyWithdraw) makes one try/catch call to
///         `volumeBot.fire()` AFTER the user's accounting has settled. The bot itself
///         alternates sell/buy on each fire so chef interactions drive 1-by-1 swap
///         volume against any configured pair. Fires are best-effort: a misconfigured
///         or out-of-funds bot must NEVER break a user's tx, hence try/catch.
contract CabalEmissionsV5 is ReentrancyGuard, Ownable {
    ...
    // ─────────────── VolumeBot ───────────────
    address public volumeBot;
    /// @dev Bot fires only when `interactionCount % volumeFireEvery == 0`. Set to 0 to disable
    ///      the hook entirely without unwiring the bot. Owner-tunable so a quiet farm can fire
    ///      every interaction (==1) and a busy farm can throttle (==10, 50, etc).
    uint256 public volumeFireEvery;
    ...
    event SetVolumeBot(address indexed bot);
    event SetVolumeFireEvery(uint256 every);
    event VolumeBotFired(address indexed user, uint256 count);
    event VolumeBotFailed(address indexed user, uint256 count);
    ...
    /// @dev Best-effort hook. Bot is non-tax-excluded so each fire generates one tax
    ///      event on the configured token. try/catch ensures a misconfigured / out-of-funds
    ///      bot can never brick a user's deposit/withdraw/harvest tx.
    ///
    ///      Fires only when `interactionCount % volumeFireEvery == 0`. `volumeFireEvery == 0`
    ///      disables the hook entirely (counter still ticks, no fire). The counter increments
    ///      on every user-facing chef call regardless of whether a fire happened.
    function _fireVolumeBot() internal {
        if (volumeBot == address(0)) return;
        uint256 c = ++interactionCount;
        if (volumeFireEvery == 0) return;
        if (c % volumeFireEvery != 0) return;
        try IVolumeBot(volumeBot).fire() {
            emit VolumeBotFired(msg.sender, c);
        } catch {
            emit VolumeBotFailed(msg.sender, c);
        }
    }
    ...
    /// @notice Wire (or unwire) the volume bot. Pass address(0) to disable the hook entirely.
    function setVolumeBot(address _bot) external onlyOwner {
        volumeBot = _bot;
        emit SetVolumeBot(_bot);
    }

    /// @notice Set the fire cadence. 0 = disabled (hook short-circuits without firing). 1 =
    ///         every interaction. N = every Nth interaction.
    function setVolumeFireEvery(uint256 _every) external onlyOwner {
        volumeFireEvery = _every;
        emit SetVolumeFireEvery(_every);
    }
